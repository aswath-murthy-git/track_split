...
def send_verification_email(email, code):
    try:
        ...
        msg['From'] = SMTP_EMAIL
        msg['To'] = email
        msg['Subject'] = "Verify Your Email - Track Split"
        
        body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;">
            <div style="max-width: 600px; margin: 0 auto; background: white; padding: 30px; border-radius: 10px;">
                <h1 style="color: #667eea; text-align: center;">Track Split</h1>
                <h2 style="color: #333;">Verify Your Email</h2>
                <p style="font-size: 16px; color: #666;">Thank you for registering! Please use the verification code below:</p>
                ...
                <p style="font-size: 12px; color: #999; text-align: center;">© 2025 Track Split</p>
            </div>
        </body>
        </html>
        """
...
def send_reset_email(email, code):
    try:
        ...
        msg['From'] = SMTP_EMAIL
        msg['To'] = email
        msg['Subject'] = "Password Reset - Track Split"
        ...
        <body style="font-family: -apple-system, sans-serif; padding: 20px; background-color: #1E1E1E; color: #F1EFEF;">
            <div style="max-width: 600px; margin: 0 auto; background: #595959; padding: 40px; border-radius: 16px; border: 1px solid #A6A6A6;">
                ...
                <p style="font-size: 12px; color: #A6A6A6; text-align: center;">Track Split</p>
            </div>
        </body>
        </html>
        """
...
def verify_email():
    ...
    if request.method == "POST":
        ...
        if datetime.now() > expiry_time:
            return render_template("verify_email.html", email=email, error="Code expired")
        ...
        if code == user_data['verification_code']:
            ...
            return render_template("login.html", success="Email verified! Please login.")
        else:
            return render_template("verify_email.html", email=email, error="Invalid code")
...
def resend_code():
    ...
    if username in pending_users:
        ...
        if send_verification_email(email, verification_code):
            return render_template("verify_email.html", email=email, success="New code sent!")
        else:
    ...
    if request.method == "POST":
        ...
        if not user_found:
            return render_template("forgot_password.html", 
                                 success="If an account exists, you will receive a reset code.")
...
def reset_password():
    ...
    if request.method == "POST":
        ...
        if not code or not new_password or not confirm_password:
            return render_template("reset_password.html", email=email, error="All fields required")
        
        if len(new_password) < 6:
            return render_template("reset_password.html", email=email, error="Password must be 6+ characters")
        
        if new_password != confirm_password:
            return render_template("reset_password.html", email=email, error="Passwords don't match")
        
        pending_users = load_pending_users()
        reset_key = f"reset_{username}"
        
        if reset_key not in pending_users:
            return render_template("reset_password.html", email=email, error="Session expired")
        
        reset_data = pending_users[reset_key]
        expiry_time = datetime.fromisoformat(reset_data['expiry_time'])
        
        if datetime.now() > expiry_time:
            return render_template("reset_password.html", email=email, error="Code expired")
        ...
        if code == reset_data['reset_code']:
            ...
            return render_template("reset_password.html", email=email, error="Invalid code")
    ...
    if request.method == "POST":
        if not SEPARATION_ENABLED:
            return render_template("index.html", username=session['username'], 
                                 is_guest=is_guest, error="Audio separation not available")
            ...
            try:
                ...
                if vocals_files and karaoke_files:
                    return render_template("result.html",
                                         vocals=os.path.basename(vocals_files[-1]),
                                         karaoke=os.path.basename(karaoke_files[-1]),
                                         username=session['username'],
                                         is_guest=is_guest)
                else:
                    return render_template("index.html", username=session['username'], 
                                         is_guest=is_guest, error="Separation failed")
            except Exception as e:
                print(f"Error: {e}")
                return render_template("index.html", username=session['username'], 
                                     is_guest=is_guest, error=f"Error: {str(e)}")
        else:
            return render_template("index.html", username=session['username'], 
                                 is_guest=is_guest, error="Please select a file")
...
def download_file(folder, filename):
    try:
        folder_path = VOCALS_FOLDER if folder == "vocals" else KARAOKE_FOLDER
